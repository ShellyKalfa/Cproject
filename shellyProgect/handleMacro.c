  ...
  char EmacrNoPunctuation[] ="macr name doesnt containing punctuation";
   ...
   if(!isalpha(nameMacr[0])){
    ...
    for(i=0;i<strlen(notInTheSE);i++){
      ...
      if(pos!= NULL){
         printf("punctuation=%c,punctuation=%d",notInTheSE[i],notInTheSE[i]);
...
line * showLinesMacro(char *line){
  int  lenght=0,result;
  char *token, *macrName;
  char EmacroNULL[] ="macro name is null problem";
  token=strtok(line," ");
   if(token == NULL){
     errorMessagesWithText(EmacroNULL,strlen(EmacroNULL),'r');
      return NULL;
    }
    result=isspace(*(token+strlen(token)-1));
    if(result){
